...
4. **レビュー（Subagent + 人）** → 保存されたドキュメントをSubagent（Agentツール）でレビューする。ユーザーも同時にレビューする。レビュー実行時は `docs/REVIEW_CRITERIA.md` の基準に従うこと。差し戻し時は `investigation.md` もレビュー対象に含める（レビュー基準: `docs/BUGFIX_STANDARD.md` セクション1の必須項目がすべて記載されているか）
