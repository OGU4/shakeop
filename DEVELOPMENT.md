...
| investigation.md | 原因分析・修正計画の履歴（イテレーションごとに追記） | ステップ7で差し戻された場合のみ作成 |

- **独立バグ案件（B-NNN新規）**: requirements.md → design.md の順で作成する。investigation.md は不要（差し戻し時に初めて作成する）
- **ステップ7差し戻し時**: investigation.md に原因分析・修正計画を追記する。requirements.md・design.md に不備があれば併せて修正する（`docs/BUGFIX_STANDARD.md` セクション3に従う）
