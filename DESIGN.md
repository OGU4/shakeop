...
> **注意: このドキュメントはプロジェクト初期に作成された構想・参考資料である。**
> 正式な開発ルールは DEVELOPMENT.md、技術スタックは docs/TECH_STACK.md、
> 各機能の設計は docs/issues/ 配下の機能設計書を参照すること。
>
> 以下の内容は将来の実装計画・設計指針として残しているが、
> 実装時は必ず各機能の機能設計書（docs/issues/F-NNN/design.md）を正とする。
...
2. [映像入力モジュール](#2-映像入力モジュール)
3. [認識パイプライン詳細](#3-認識パイプライン詳細)
4. [ゲーム状態管理 (FSM)](#4-ゲーム状態管理-fsm)
5. [音声通知システム](#5-音声通知システム)
6. [GUIデザイン仕様](#6-guiデザイン仕様)
7. [設定・データモデル](#7-設定データモデル)
8. [モデル学習ガイド](#8-モデル学習ガイド)
9. [クロスプラットフォーム対応メモ](#9-クロスプラットフォーム対応メモ)
10. [付録: 武器アイコン分類（将来対応メモ）](#付録-武器アイコン分類将来対応メモ)
...
### 3.2 Stage 1: シーン分類 (Scene Classifier)
...
### 3.3 Stage 2: オオモノシャケ検出 (Object Detector)
...
### 3.4 Stage 3: 数字認識 (Digit Recognizer)
...
### 3.5 Stage 3b: 固定テキスト識別 (Text Identifier)
...
### 3.6 Stage 4: 特徴量マッチング (Feature Matcher)
...
| 軽量CNN分類 (MobileNet等) | 良好 | 各50-100枚 | 確実だが学習必要 |
| メトリック学習 (Triplet Loss) | 優秀 | 各5-10枚 | 少量データで亜種区別に強い |
