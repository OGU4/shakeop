#!/usr/bin/env python3
"""
exp_007: "Clear!!" リザルト画面認識 (pHash 1段判定)
...
実行:
  ...
  uv run python experiments/exp_007_clear_result_recognition/main.py \
      --create-template --image data/test_fixtures/clear_result/positive/<画像>
...
# テンプレートファイル名
TEMPLATE_FILE = "clear_result.npy"

# 画像ファイル拡張子
IMAGE_EXTENSIONS = {".png", ".jpg", ".jpeg", ".bmp"}


def create_recognizer(template_dir: Path, threshold: int) -> ClearResultRecognizer:
...
def run_create_template(args: argparse.Namespace) -> None:
    ...
    # ROI切り出し → グレースケール変換
    x1, y1, x2, y2 = ClearResultRecognizer.CLEAR_RESULT_ROI
    roi = frame[y1:y2, x1:x2]
    gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)

    # pHash計算 & 保存
    clear_hash = compute_phash(gray)
    save_path = template_dir / TEMPLATE_FILE
    np.save(str(save_path), clear_hash)
    print(f"テンプレート保存: {save_path}")
    print(f"  Clear!! ハッシュ: {clear_hash.flatten()}")

    if args.debug:
        print("\n--- デバッグ情報 ---")
        print(f"画像: {args.image}")
        print(f"ROI座標: {ClearResultRecognizer.CLEAR_RESULT_ROI}")
...
def run_image_dir(args: argparse.Namespace) -> None:
    ...
    if args.debug:
        ...
        for category in ["positive", "negative"]:
            cat_results = [
                r
                for r in results
                if r["path"].parent.name == category and r["debug_info"]
            ]
            if cat_results:
    ...
    result_file = results_dir / f"{timestamp}_clear_result.txt"

    with open(result_file, "w") as f:
        f.write("# F-007 Clear!! Recognition Result\n")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='exp_007: "Clear!!" リザルト画面認識 (pHash 1段判定)'
    )
    ...
    parser.add_argument(
        "--image", type=str, help="単一画像パス（テンプレート作成時または判定時）"
    )
...
def main() -> None:
    ...
    if args.create_template:
        ...
        print(
            "Error: --create-template, --image-dir, または --image を指定してください"
        )
        print()
        print("テンプレート作成:")
        print("  uv run python experiments/exp_007_clear_result_recognition/main.py \\")
        print("      --create-template --image <Clear!!表示の画像> --debug")
        print()
        print("単一画像判定:")
        print("  uv run python experiments/exp_007_clear_result_recognition/main.py \\")
        print("      --image <判定対象画像>")
        ...
        print("ディレクトリ一括判定:")
        print("  uv run python experiments/exp_007_clear_result_recognition/main.py \\")
        print("      --image-dir data/test_fixtures/clear_result/ --debug")
