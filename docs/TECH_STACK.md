...
>
> **最終更新**: 2026-03-04（ソース: pyproject.toml, DESIGN.md, CLAUDE.md, docs/issues/*, 実コードimport文）
...
| 項目 | 値 | ソース |
|---|---|---|
| 言語 | Python | pyproject.toml, DESIGN.md |
| バージョン指定 | `>=3.12`（pyproject.toml で定義） | pyproject.toml, CLAUDE.md, DESIGN.md |
| パッケージ管理 | uv（pyproject.toml ベース、lockfile対応） | CLAUDE.md, DESIGN.md |
| 対象OS | Windows / Linux（クロスプラットフォーム） | DESIGN.md 付録C |
...
| onnxruntime | `>=1.17` | ONNX推論（Scene分類、Object検出） | **未使用**（実コードにimportなし。DESIGN.md で将来の認識パイプラインに使用予定） | CPU推論前提、GPU非搭載PCでも動作可能 |
| tomli-w | `>=1.0` | TOML書き出し（設定ファイル出力用） | **未使用**（実コードにimportなし。DESIGN.md の設定管理で使用予定） | pyproject.tomlと同じTOML形式で設定管理 |
...
| 禁止対象 | 理由 | ソース |
|---|---|---|
| 汎用OCR（Tesseract, PaddleOCR 等） | ゲーム画面の独自フォント・動く背景・透過・斜め配置に不向き。pHashで十分な精度が得られている | DESIGN.md, F-001, F-004 設計書 |
...
| 制約 | 内容 | ソース |
|---|---|---|
| 解像度 | FHD (1920x1080) キャプチャ入力を前提。すべてのROI座標はFHD基準のピクセル座標 | DESIGN.md, 全要求仕様書 |
...
| 推論環境 | CPU推論前提（GPU非搭載PCでも動作可能にする） | DESIGN.md |
| 音声形式 | QSoundEffect は `.wav` のみ対応（mp3不可）。44.1kHz/16bit PCM推奨 | DESIGN.md 付録C |
