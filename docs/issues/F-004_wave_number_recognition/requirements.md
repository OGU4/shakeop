...
**ステップ3: 共通化リファクタリング**
- ステップ1で実装した `WaveNumberRecognizer` の `compute_phash` と `hamming_distance` を `shared/recognition/` に切り出す
- F-005 以降の認識機能から再利用可能にすることが目的
- 認識ロジック・精度に変更なし（リファクタリングのみ）
- `WaveNumberRecognizer` は `shared/recognition/` の関数を使うように改修する
...
- [ ] `shared/recognition/` に `compute_phash` と `hamming_distance` が配置されていること
- [ ] `WaveNumberRecognizer` が `shared/recognition/` の関数を使用していること
- [ ] 改修後の精度がステップ1と同一であること（回帰なし）
- [ ] GUI統合版（ステップ2）が改修後も正常動作すること
